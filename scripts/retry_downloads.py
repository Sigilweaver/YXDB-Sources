...
from state import load_known_repos, save_known_repos
...
def main():
    ...
    for repo, info in sorted(e2_repos.items()):
        ...
        for ef in info["e2_files"]:
            safe = repo.replace("/", "_") + "_" + os.path.basename(ef["path"])
            dest = os.path.join(DOWNLOADS_DIR, safe)
            if not os.path.exists(dest):
                missing.append((repo, branch, ef, dest))
    ...
    for i, (repo, branch, ef, dest) in enumerate(missing, 1):
        path = ef["path"]
        ...
        if "!/" in path:
            ...
            file_data = download_raw(repo, path, branch)
